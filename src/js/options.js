...
function init(){
    ...
    // Initialize the new tab toggle functionality
    initNewTabToggle();
}
...
// --- NEW FEATURE: NEW TAB TOGGLE START ---
function initNewTabToggle() {
const newTabToggle = document.getElementById('newTabToggle');
  if (!newTabToggle) return; // Exit if the element isn't on the page

  // Load the saved setting from chrome.storage.sync
    chrome.storage.sync.get(['newTabEnabled'], (result) => {
    // Default to 'true' (enabled) if the setting doesn't exist yet
    newTabToggle.checked = typeof result.newTabEnabled === 'undefined' ? true : result.newTabEnabled;
});

  // Add a listener to save the setting when the user clicks the toggle
    newTabToggle.addEventListener('change', () => {
    const isEnabled = newTabToggle.checked;
    chrome.storage.sync.set({ newTabEnabled: isEnabled }, () => {
    console.log('New Tab setting saved:', isEnabled);
    });
});
}
// --- NEW FEATURE: NEW TAB TOGGLE END ---